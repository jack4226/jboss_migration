...
public interface MessageInboxRepository extends JpaRepository<MessageInbox, Integer>,
	...
	@Modifying(clearAutomatically = true)
	@Query(nativeQuery=true, value="update message_inbox set updt_user_id = ?3, updt_time = ?4, status_id = 'C', messagefolderrowid = ("
			+ "select row_id from message_folder where foldername = ?2) where leadmsgrowid = ?1")
	public int closeMessagesByLeadMsgId(Integer leadMsgId, String folderName, String updtUserId, java.sql.Timestamp time);
	
	@Modifying(clearAutomatically = true)
	@Query(nativeQuery=true, value="update message_inbox set updt_user_id = ?4, updt_time = ?5, status_id = ?3, messagefolderrowid = ("
			+ "select row_id from message_folder where foldername = ?2) where row_id = ?1")
	public int moveMessageToAnotherFolder(Integer msgId, String folderName, String statusId, String updtUserId, java.sql.Timestamp time);
